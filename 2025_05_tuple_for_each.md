...
```cpp
const auto fn = [](const auto& value) { std::cout << value << ' '; };
...
How can we implement this before a function like `std::tuple_for_each` becomes part of the standard, and without relying on third-party libraries like `Boost`? Let's explore several different approaches. Each of these implementations can be useful depending on specific variations of the problem.
...
In this version, `std::apply(func, tuple)` unpacks the tuple as arguments to `func`, i.e., `(std::get<0>(tuple), std::get<1>(tuple), ...)`, and the fold expression turns that into a sequence of `fn` calls.
